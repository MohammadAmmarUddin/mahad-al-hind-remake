import { useState } from "react";

const WHATSAPP_NUMBER = "919365262648";
const WHATSAPP_URL = `https://api.whatsapp.com/send?phone=${WHATSAPP_NUMBER}`;

const Whatsapp = () => {
  const [isExpanded, setIsExpanded] = useState(false);

  const handleClick = () => {
    setIsExpanded(!isExpanded);
  };

  const openWhatsApp = () => {
    window.open(WHATSAPP_URL, "_blank");
  };

  return (
    <div className="fixed z-50 bottom-10 right-10">
      {isExpanded ? (
        <div className="bg-white rounded-lg overflow-hidden w-64 transition-all duration-300 ease-in-out custom-shadow border p-4">
          <div className="flex justify-between items-center mb-4">
            <h3 className="text-green-600 font-bold">Chat with us</h3>
            <button
              onClick={handleClick}
              className="text-gray-500 hover:text-gray-700"
            >
              ✕
            </button>
          </div>
          <div className="flex flex-col items-center">
            <p className="mb-2 text-sm text-gray-600">Scan to chat with us</p>
            <div className="p-2 rounded-md mb-2">
              <div className="w-48 mb-5 flex items-center justify-center">
                <img
                  src="./qr-code.png"
                  alt="WhatsApp QR Code"
                  className="w-full object-cover border"
                />
              </div>
            </div>
            <button
              onClick={openWhatsApp}
              className="w-full rounded-lg bg-green-600 py-2.5 text-sm font-semibold text-white transition hover:bg-green-700"
            >
              Chat Now
            </button>
          </div>
        </div>
      ) : (
        <button onClick={openWhatsApp} className="block">
          <img
            className="w-12 h-12 cursor-pointer animate-pulse drop-shadow-lg"
            src="./whatsapp-logo.png"
            alt="WhatsApp"
          />
        </button>
      )}
    </div>
  );
};

export default Whatsapp;
